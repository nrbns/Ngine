...
// Ensure the supabase module is mocked so we can control realtime subscriptions in tests
jest.mock('../../services/supabase', () => ({
  ...(jest.requireActual('../../services/supabase') as object),
  subscribeToGoalProofs: jest.fn(),
}));
...
describe('ProofGallery (mock mode)', () => {
  ...
  it('shows empty state and adds a proof via gallery', async () => {
    ...
    // Also mock realtime subscription to simulate a server-side insert later
    const sup = require('../../services/supabase');
    jest.useFakeTimers();
    jest.spyOn(sup, 'subscribeToGoalProofs').mockImplementation((id: string, cb: any) => {
      // call back with a new proof after a short delay
      setTimeout(() => cb({ eventType: 'INSERT', new: { id: 'remote-1', file_url: 'file://remote.jpg', file_type: 'image', created_at: new Date().toISOString() } }), 500);
      return {} as any;
    });
    ...
    // Advance timers to trigger the mocked realtime insert
    await act(async () => {
      jest.advanceTimersByTime(600);
    });

    // The toast should be visible with the live notification
    await waitFor(() => expect(getByText('New proof added')).toBeTruthy());
